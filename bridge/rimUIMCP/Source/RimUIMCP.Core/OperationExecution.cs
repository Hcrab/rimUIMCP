...
using RimUIMCP.Contracts;

namespace RimUIMCP.Core;
...
public sealed class OperationRunner
{
    ...
    private static OperationError CreateError(Exception exception, string code)
    {
        ...
        if (IsLikelySdkCompatibilityException(exception, root))
        {
            return new OperationError
            {
                ...
                Message = "A companion tool was built against a different RimUIMCP.Sdk API than the running RimUIMCP host provides. Rebuild/redeploy RimUIMCP and the companion mod, then restart RimWorld so both load the same SDK version.",
    ...
    private static bool ContainsSdkName(string value)
    {
        return string.IsNullOrWhiteSpace(value) == false
            && value.IndexOf("RimUIMCP.Sdk", StringComparison.OrdinalIgnoreCase) >= 0;
    }
}
