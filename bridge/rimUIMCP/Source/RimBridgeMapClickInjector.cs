...
internal static class RimBridgeMapClickInjector
{
    ...
    private static MapClickRequest QueueRequest(IntVec3 clickCell, IntVec3 endCell, string targetLabel, MapClickDispatchOptions options)
    {
        ...
        var request = new MapClickRequest
        {
            ...
            ScreenPositionInverted = options.ScreenPositionInverted ?? RimWorldState.CellCenter(clickCell).MapToUIPosition(),
            EndScreenPositionInverted = endCell.IsValid ? RimWorldState.CellCenter(endCell).MapToUIPosition() : options.ScreenPositionInverted ?? RimWorldState.CellCenter(clickCell).MapToUIPosition(),
