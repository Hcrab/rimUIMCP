    ...
    private static void TryStart()
    {
        ...
        try
        {
            using (RimBridgeStartupTiming.Phase("bridge-start.total"))
            {
                ...
                var version = typeof(RimUIMCPMod).Assembly.GetName().Version?.ToString() ?? "0.1.0.0";
                var builder = Lib.GAB.Gabp.CreateServer();
                using (RimBridgeStartupTiming.Phase("gabp.builder.configure"))
                {
                    builder = builder
                        .UseAppInfo("RimUIMCP", version)
                ...
                {
                    _server.Tools.RegisterToolsFromInstance(new RpTools());
                    if (RpSession.FixtureMode) {
