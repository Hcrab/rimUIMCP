...
internal static class RimBridgeUiWorkbench
{
    ...
    {
        public RootPointerState PointerState { get; set; }
        public string SurfaceTargetId { get; set; } = string.Empty;
    ...
    public static object ClickUiTargetResponse(string targetId, int timeoutMs = 2000, int button = 0, string action = "click", string text = null, EventModifiers modifiers = EventModifiers.None)
    {
        timeoutMs = timeoutMs <= 0 ? 2000 : timeoutMs;
        ...
        try
        {
            request = RimBridgeMainThread.Invoke(() => { var queued = QueueClick(targetId); queued.Button = button; queued.Action = action; queued.Text = text; queued.Modifiers = modifiers; return queued; }, timeoutMs: 5000);
        }
        catch (Exception ex)
        {
            return new
    ...
    public static void BeginCustomSurface(string id, string type)
    {
        BeginSurface(new SurfaceDescriptor { SurfaceTargetId = id, SurfaceKind = "custom", Type = type,
            Rect = ToSnapshot(new Rect(0, 0, UI.screenWidth, UI.screenHeight)) });
    }

    public static bool TryGetFormInput(UiPatchControlState state, out string text)
    {
        lock (Sync) {
            text = _pendingClick?.Text;
            return state?.ShouldActivate == true && _pendingClick != null
                && (_pendingClick.Action == "fill" || _pendingClick.Action == "setValue");
        }
    }

    public sealed class RootPointerState { public Event Previous; public int Token; }
    public static RootPointerState BeginRootPointer()
    {
        lock (Sync) {
            Vector2 point;
            if (_pendingClick?.ScreenRect != null) {
                var rect = _pendingClick.ScreenRect;
                point = new Vector2(rect.X + rect.Width / 2, rect.Y + rect.Height / 2);
            } else if (!RimBridgeVirtualPointer.TryGetMousePositionOnUiInverted(out point)) return null;
            var state = new RootPointerState { Previous = Event.current, Token = RimBridgeVirtualPointer.PushTransientOverride(point) };
            // Set the pointer before IMGUI builds scroll clips. Preserve this draw event's type.
            var current = Event.current == null ? new Event() : new Event(Event.current);
            current.mousePosition = point * Prefs.UIScale;
            Event.current = current;
            return state;
        }
    }
    public static void EndRootPointer(RootPointerState state)
    {
        if (state == null) return;
        Event.current = state.Previous;
        RimBridgeVirtualPointer.PopTransientOverride(state.Token);
    }

    public static void CancelPendingInput()
    {
        lock (Sync) {
            if (_pendingClick != null) CancelClick(_pendingClick.TargetId);
        }
    }
    ...
    private static void BeginSurface(SurfaceDescriptor descriptor)
    {
        ...
        lock (Sync)
        {
            ...
            RootPointerState pointerState = null;
            Vector2 pointerPoint;
            if (click?.ScreenRect != null) {
                pointerPoint = new Vector2(click.ScreenRect.X + click.ScreenRect.Width / 2, click.ScreenRect.Y + click.ScreenRect.Height / 2);
            } else if (!RimBridgeVirtualPointer.TryGetMousePositionOnUiInverted(out pointerPoint)) pointerPoint = new Vector2(float.NaN, float.NaN);
            if (!float.IsNaN(pointerPoint.x)) {
                pointerState = new RootPointerState { Previous = Event.current, Token = RimBridgeVirtualPointer.PushTransientOverride(pointerPoint) };
                var pointerEvent = Event.current == null ? new Event() : new Event(Event.current);
                pointerEvent.mousePosition = pointerPoint - UI.GUIToScreenPoint(Vector2.zero);
                Event.current = pointerEvent;
            }
            if (capture == null && click == null && scroll == null && hover == null)
            {
                SurfaceStack.Push(new LiveSurfaceState { PointerState = pointerState });
                return;
            }
    ...
    public static void EndSurface()
    {
        lock (Sync)
        {
            if (SurfaceStack.Count > 0)
                EndRootPointer(SurfaceStack.Pop()?.PointerState);
        }
    }
    ...
    public static void BeginContainer(string kind, string source, Rect rect, string label = null)
    {
        lock (Sync)
        {
            ...
            surface.ContainerTargetIds.Push(captureTargetId ?? "live-container");
        }
    }
    ...
    public static void BeginScrollViewContainer(Rect viewportRect, ref Vector2 scrollPosition, Rect contentRect)
    {
        lock (Sync)
        {
            ...
            surface.ContainerTargetIds.Push(captureTargetId ?? "live-scroll");
        }
    }
    ...
    public static void EndContainer()
    {
        ...
        {
            var surface = GetCurrentSurface();
            if (surface == null || surface.CompoundDepth > 0 || surface.ContainerTargetIds.Count == 0)
                return;
    ...
    public static void PrepareControlInteraction(UiPatchControlState state, Rect rect, Vector2? pointer = null)
    {
        ...
        lock (Sync)
        {
            ...
            var point = pointer ?? rect.center;
            var pointerInverted = UI.GUIToScreenPoint(point);
            var currentEvent = Event.current;
            var injectedEvent = currentEvent == null ? new Event() : new Event(currentEvent);
            injectedEvent.mousePosition = point;

            if (state.ShouldActivate && _pendingClick != null)
            {
                RimBridgeVirtualPointer.SetPersistentPointer("ui-input", _pendingClick.TargetId, _pendingClick.Label,
                    pointerInverted, null, durationMs: 60000);
                injectedEvent.type = _pendingClick.Phase == ClickPhase.MouseDown ? EventType.MouseDown : EventType.MouseUp;
                injectedEvent.button = _pendingClick.Button;
                injectedEvent.modifiers = _pendingClick.Modifiers;
    ...
    public static EventType EventTypeForActiveControl(Event current)
    {
        var type = current.type;
        if (type != EventType.Ignore) return type;
        lock (Sync) {
            if (_pendingClick == null || _activeInteraction?.ShouldActivate != true || current != Event.current) return type;
            var box = _activeInteraction.Rect;
            if (Mouse.IsOver(new Rect(box.X, box.Y, box.Width, box.Height))) return current.rawType;
        }
        return type;
    }

    public static bool IsReleasingControl(UiPatchControlState state) => state?.ShouldActivate == true && _pendingClick?.Phase == ClickPhase.MouseUp;

    public static bool MouseButtonForActiveControl(int button)
    {
        lock (Sync) {
            if (_pendingClick != null && _activeInteraction?.ShouldActivate == true && _pendingClick.Button == button)
                return _pendingClick.Phase == ClickPhase.MouseDown && !Mouse.IsInputBlockedNow;
        }
        return Input.GetMouseButton(button);
    }
    public static bool MouseDragForActiveControl(int button = 0)
    {
        lock (Sync) {
            if (_pendingClick != null && _activeInteraction?.ShouldActivate == true && _pendingClick.Button == button)
                return _pendingClick.Phase == ClickPhase.MouseDown && !Mouse.IsInputBlockedNow;
    ...
    public static void OverrideButtonResultIfPending(UiPatchControlState state, ref bool result)
    {
        ...
        lock (Sync)
        {
            if (_pendingClick == null || _pendingClick.Activated || _pendingClick.Button != 0)
                return;
            ...
            if (!RectApproximatelyEquals(_activeInteraction.Rect, state.InteractionRect, 1f))
                return;
            if (!GUI.enabled || Mouse.IsInputBlockedNow) return;
        ...
        UiLayoutScrollSnapshot scroll = null)
    {
        ...
        var normalizedValueText = kind == "text_field" ? valueText ?? string.Empty : NormalizeText(valueText);
        ...
        if (surface.CaptureSurface != null)
        {
            ...
            {
                SemanticName = RpUiContext.Current?.Name,
                ActionId = RpUiContext.Current?.ActionId,
                OwnerId = RpUiContext.Current?.OwnerId,
                RowKey = RpUiContext.Current?.RowKey,
                ElementIndex = elementIndex,
                TargetId = targetId,
                Kind = kind,
                Source = source,
    ...
    private static bool ShouldActivateCurrentElement(LiveSurfaceState surface, string kind, string source, Rect rect, string label, bool actionable, bool? disabled)
    {
        ...
        if (_pendingClick.OwnerId != RpUiContext.Current?.OwnerId || _pendingClick.RowKey != RpUiContext.Current?.RowKey)
            return false;
        return _pendingClick.ElementIndex == surface.ElementIndex && _pendingClick.Depth == surface.ContainerTargetIds.Count
            && RectApproximatelyEquals(_pendingClick.Rect, rect, 1f)
            && RectApproximatelyEquals(_pendingClick.ScreenRect, ToScreenSnapshot(rect), 1f);
    }
...
{
    public static void Prefix(Rect position, string text, out RimBridgeUiWorkbench.UiPatchControlState __state) => __state = RpFormAdapter.Begin(position, text);
    public static void Postfix(ref string __result, RimBridgeUiWorkbench.UiPatchControlState __state) => RpFormAdapter.End(__state, ref __result);
}
...
{
    public static void Prefix(Rect position, string text, out RimBridgeUiWorkbench.UiPatchControlState __state) => __state = RpFormAdapter.Begin(position, text);
    public static void Postfix(ref string __result, int maxLength, RimBridgeUiWorkbench.UiPatchControlState __state) => RpFormAdapter.End(__state, ref __result, maxLength);
}
...
internal static class Widgets_CheckboxLabeled_UiWorkbench_Patch
{
    ...
    [HarmonyPriority(Priority.First)]
    public static void Prefix(Rect rect, string label, ref bool checkOn, bool disabled, bool placeCheckboxNearText, ref RimBridgeUiWorkbench.UiPatchControlState __state)
    {
        // CheckboxLabeled narrows its actual hit target when the checkbox follows the text.
        // Capture the same rectangle so our input reaches the original toggle handler.
...
// Both Listing_Standard.CheckboxLabeled overloads delegate to the captured
// Widgets.CheckboxLabeled. Let that child supply its actual height and local
// hit rectangle; predicting one here double-counts nested listing offsets.

// Listing_Standard.ButtonText delegates to Widgets.ButtonText, which is already
// captured above. Capture that actual widget rectangle: predicting it here adds
// listingRect twice inside the GUI group and suppresses the correctly placed child.
