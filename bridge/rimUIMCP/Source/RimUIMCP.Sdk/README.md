...
Compile-time SDK for RimUIMCP companion tool assemblies.

Use this package from a companion DLL that is loaded by RimUIMCP at runtime from a `BridgeTools` folder. The package supplies the `[Tool]` annotations and the optional async runtime interfaces that companion tools can request as injected parameters.
...
RimUIMCP resolves this assembly to the copy shipped by the running mod. Companion projects should reference this package for compilation, but should not deploy `RimUIMCP.Sdk.dll` beside the companion DLL.
...
If a companion tool fails after rebuilding against a different SDK shape, call `rimbridge/get_bridge_status` in the running game. RimUIMCP reports the host SDK version and per-companion discovery diagnostics, including local SDK-copy warnings and load/register errors.
