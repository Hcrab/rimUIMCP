...
internal static class RimWorldTickStepper
{
    ...
    private static StepSnapshot AdvanceLocked(StepRequest request, int frameCount)
    {
        ...
        try
        {
            ...
            // Paused TickRateMultiplier is zero, so the ordinary render tween does
            // not follow manually stepped movement. Refresh only the visual root.
            if (Find.CurrentMap is Map drawMap)
                foreach (var pawn in drawMap.mapPawns.AllPawnsSpawned)
