---
name: rimuimcp-companion-tools
description: Use when adding, migrating, reviewing, or testing RimUIMCP 2.x companion DLL tools for a RimWorld mod. Covers RimUIMCP.Sdk package references, BridgeTools folder layout, companion csproj wiring, async IRimBridgeContext tool orchestration, local NuGet/package use, build/deploy automation, and GABS validation of mod-owned [Tool] methods.
---

# RimBridge Companion Tools

Use this skill for source work in a RimWorld mod that wants to expose tools through RimUIMCP 2.x. Do not use it for ordinary live bridge calls; use the `rimuimcp` skill for running-game interaction.
...
3. Reference `RimUIMCP.Sdk` for compile-time annotations and runtime interfaces, but do not deploy `RimUIMCP.Sdk.dll` with the companion. RimUIMCP resolves it to the host copy.
...
- Do not keep new public `[Tool]` methods in the main mod assembly when migrating to RimUIMCP 2.x; enabled mods are discovered through explicit `BridgeTools` folders.
