...
Use this reference when adding or migrating RimUIMCP 2.x companion tools in an existing RimWorld mod.

## Load Model

- RimUIMCP starts late enough that normal mod assemblies are already loaded.
- RimUIMCP then loads companions from explicit `BridgeTools` folders.
...
- `RimUIMCP.Sdk` always resolves to the SDK assembly shipped by RimUIMCP. Companion projects reference the SDK for compilation, but should not deploy that DLL.
...
Create the local SDK package from the RimUIMCP checkout:
...
```text
../.nuget-local/RimUIMCP.Sdk.2.0.0.nupkg
```
...
```xml
<PackageReference Include="RimUIMCP.Sdk" Version="2.0.0" PrivateAssets="all" ExcludeAssets="runtime" />
```

When the companion project lives next to a RimUIMCP checkout during early development, a `ProjectReference` is also acceptable:

```xml
<ProjectReference Include="..\..\..\RimUIMCP\Source\RimUIMCP.Sdk\RimUIMCP.Sdk.csproj" Private="false" />
```

Keep `Private="false"` or `ExcludeAssets="runtime"` so the companion output does not bundle `RimUIMCP.Sdk.dll`.
...
- Avoid arbitrary sleeps. Let RimUIMCP advance frames/ticks through the SDK.
...
- Confirm `RimUIMCP.Sdk.dll` is not copied into the deployed companion bundle folder.
