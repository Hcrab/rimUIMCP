...
Build RimUIMCP and deploy it through the repo's MSBuild CopyToRimworld/ZipMod targets.

Options:
      --mods-dir <dir>        Parent Mods folder. Deploys to <dir>/RimUIMCP and <dir>/RimUIMCP.zip
...
if [[ "$RUN_TESTS" == true ]]; then
	dotnet test "$ROOT/RimUIMCP.sln" -c "$CONFIGURATION" --no-build
fi
...
if [[ -n "$TARGET_DIR" ]]; then
	...
	echo "deployed=$MODS_DIR/RimUIMCP"
	echo "zip=${ZIP_PATH:-$MODS_DIR/RimUIMCP.zip}"
fi
