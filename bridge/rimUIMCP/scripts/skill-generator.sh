...
if mentions_rimworld:
    overview_line = (
        "RimUIMCP turns a running RimWorld session into a live automation bridge. "
        "Use this skill when the goal is to inspect, drive, verify, or script RimWorld "
        "through that bridge. Do not use it for source-only editing of RimUIMCP itself."
    )
...
if has_direct:
    quick_start_lines.append(
        "10. If a direct RimUIMCP MCP connector is already installed in the current Codex session, start with its own status and discovery tools such as `rimbridge/get_bridge_status`, `rimbridge/list_capabilities`, and `rimbridge/wait_for_game_loaded`."
    )
else:
    quick_start_lines.append(
...
skill_md.extend(
    [
        ...
        *quick_start_lines,
        ...
        "## Discovery rules",
        "",
        "- Do not invoke this skill just because the current repository is RimUIMCP. Use it only when the task needs live game or bridge interaction.",
        ...
        "## Workflow",
        "",
        "1. Decide whether the task actually requires live bridge interaction. If the work is source-only editing, review, or refactoring inside RimUIMCP, do not use this skill.",
