...
Install RimUIMCP Codex skills into the local Codex skills directory.

Options:
  --install-root <dir>       Install root. Default: ${CODEX_HOME:-$HOME/.codex}/skills
  --repo-only                Install only repo-owned skills from ./skills
  --generated-only           Install only the generated rimuimcp live bridge skill
...
if [[ "$INSTALL_GENERATED" == 1 ]]; then
	"$ROOT/scripts/skill-generator.sh" --skill-name rimuimcp --install-root "$INSTALL_ROOT"
fi
