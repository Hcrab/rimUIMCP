...
This is the in-game bridge shipped with **rimUIMCP**. It is derived from
[RimBridgeServer](https://github.com/pardeike/RimBridgeServer) 2.1.1 by Andreas
Pardeike. The original project remains under its MIT license in `LICENSE`; new
rimUIMCP material is covered by `LICENSE-rimUIMCP`. See
`THIRD_PARTY_NOTICES.md` and `UPSTREAM.md` for attribution and provenance.

Portions of the materials used to create this content/mod are trademarks and/or copyrighted works of Ludeon Studios Inc. All rights reserved by Ludeon. This content/mod is not official and is not endorsed by Ludeon.

## Bridge documentation

rimUIMCP is a RimWorld mod that turns the running game into a bridge an external tool can use.
...
rimUIMCP runs inside RimWorld and exposes a tool surface for:
...
GABS is the best way to use rimUIMCP from an AI or automation harness.
...
2. `rimUIMCP` inside RimWorld for live game inspection and control
...
When [Dubs Performance Analyzer](https://steamcommunity.com/sharedfiles/filedetails/?id=2038874626) is installed and active, rimUIMCP also exposes generic DPA profiling tools for repeatable method-level Tick and Update measurements.
...
In direct mode, rimUIMCP logs lines like:
...
For cross-repo development in a sibling workspace, rimUIMCP supports an opt-in local NuGet source override without switching the project to `ProjectReference`s.

- default local source path: `../.nuget-local`
- enable it with `EnableLocalNuGetOverride=true`
- override the local Lib.GAB package version with `LibGabPackageVersion`

Example:

```bash
dotnet restore Source/RimUIMCP.csproj \
...
Pack the rimUIMCP companion SDK into that same local source with:
...
That command refreshes the generated `rimuimcp` live bridge skill from `README.md` and `docs/tool-reference.md`, then installs repo-owned skills from `skills/`, including `rimuimcp-companion-tools` for adding `RimUIMCP.Sdk` companion DLLs to RimWorld mods.

## Local Deploy Targets

`RIMWORLD_MOD_DIR` is a parent directory deploy target: builds copy the mod to `$RIMWORLD_MOD_DIR/rimUIMCP` and write `$RIMWORLD_MOD_DIR/rimUIMCP.zip`.
...
Third-party mods can expose bridge tools through companion DLLs that reference the `RimUIMCP.Sdk` NuGet package. rimUIMCP starts its bridge services after RimWorld has loaded normal mod assemblies, then loads companion DLLs from explicit `BridgeTools` folders and registers their `[Tool]` methods through the same capability registry and top-level GAB tool surface as built-in tools.

Practical rules:

- use `RimUIMCP.Sdk` as the only shared bridge dependency
...
public sealed class MyModBridgeTools
{
    [Tool(
        "mymod/ping",
        Description = "Example tool exposed through rimUIMCP",
...
Playback default: rimUIMCP enables RimWorld's private `TickManager.UltraSpeedBoost` process-wide at startup. Speed 4 (`Ultrafast`) therefore uses the boosted debug rate by default until `rimworld/set_time_speed` changes the flag; the next RimWorld startup enables it again.
...
- `rimworld/set_time_speed` - Set RimWorld's current time speed directly; rimUIMCP enables the boost for speed 4 (Ultrafast) at startup, and this tool can change that state
...
- `rimworld/set_camera_zoom_extension` - Enable or disable rimUIMCP's session-only extended camera zoom range; disabled by default
